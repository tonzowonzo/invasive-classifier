...
def _bbox_area_curve(
    boxes: Dict[int, Tuple[int, int, int, int]], frames: Sequence[int]
) -> np.ndarray:
    areas = []
    for f in frames:
        b = boxes.get(int(f))
...
def track_debug_panel(
    ...
    inner_gs = GridSpec(
        ...
        top=ax_grid.get_position(fig).y1,
        wspace=0.05,
        hspace=0.15,
    )
    ...
    if video_path is not None and boxes is not None:
        ...
        sub_gs = GridSpec(
            ...
            top=ax_raw.get_position(fig).y1,
            wspace=0.05,
        )
        ...
        ax_placeholder.text(
            0.5, 0.5, "raw frames unavailable", ha="center", va="center", fontsize=9
        )
    ...
    if class_probs is not None and class_names is not None:
        ...
        ax_p.set_yticks(range(k))
        ax_p.set_yticklabels(labs, fontsize=8)
        ax_p.set_xlim(0, 1.0)
...
def batch_grid(
    ...
    for i in range(B):
        ok = preds[i] == gts[i]
        if correct_only is True and not ok:
            continue
        if correct_only is False and ok:
            continue
        idxs.append(i)
    idxs = idxs[:max_items]
    if not idxs:
        ...
        ax.text(0.5, 0.5, "No items for this filter", ha="center", va="center")
        ax.axis("off")
        return fig
...
def plot_class_distribution(
    label_list: List[str], top_n: int = 30, title: str = "Class distribution"
):
...
def prediction_histogram(
    preds: np.ndarray, class_names: List[str], title: str = "Predictions per class"
):
...
def pca_scatter(
    ...
    for cid in np.unique(targets):
        ...
        ax.scatter(
            ...
            s=8,
            alpha=0.6,
            label=_shorten(class_names[int(cid)], 20),
        )
    ax.set_title(title)
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
